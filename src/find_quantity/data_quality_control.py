...
from find_quantity.model import Month, Product
...
class ValidateProductQuantity:
    ...
    def calc_stock_diff(self) -> int:
        return self.raw_data_stock_initial - (
            self.calc_stock_qt + self.calc_all_units_sold
        )
...
def validate_calculated_products(
    calculation_report: dict,
...
def validate_extracted_product_raw_data(
    raw_products: dict[Month, list[Product]],
) -> list[ProductRawDataSimplified]:
    cleaned_products: dict[Month, dict[str, list]] = defaultdict(
    ...
    for month, name_products in cleaned_products.items():
        for product_name, products in name_products.items():
            ...
            v = ProductRawDataSimplified(
                month=month, product_name=product_name, stock_qt_initial=stock_qt_intial
            )
...
def product_validation(
    validation_data_product_calc: list[ValidateProductQuantity],
