...
class PackageConstractor:
    ...
    def construct_packages(self) -> list[Package]:
        ...
        for p in self.get_products():
            pk = Package(sub_products=[p], n_article=f'PKG-{p.n_article}', stock_lmt=self.products[p])
            packages.append(pk)
...
def test_not_enough_quantity_to_make_package_with_incomplete_packages_allowed():
    ...
    pks = PackageConstractor(products, package_definitions, allow_incomplete_packages=True).construct_packages()
    assert len(pks) == 1
...
def test_not_enough_quantity_to_make_package_with_incomplete_packages_not_allowed():
    ...
    pks = PackageConstractor(products, package_definitions, allow_incomplete_packages=False).construct_packages()
    assert len(pks) == 3
